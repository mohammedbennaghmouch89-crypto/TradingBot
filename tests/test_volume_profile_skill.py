"""Validation tests for the Volume Profile skill (`.claude/skills/volume-profile/`).

Like the ICT skill, this is documentation, so these tests guard the skill's
*structural integrity* (valid Claude Code skill, present reference files, working
internal links, key conventions retained) rather than trading behavior. Stdlib
only, so they pass in CI even when third-party packages cannot be installed. See
``docs/TESTING.md``.
"""
...
SKILL_DIR = REPO_ROOT / ".claude" / "skills" / "volume-profile"
SKILL_MD = SKILL_DIR / "SKILL.md"

REQUIRED_REFERENCES = (
    "reference/foundations-and-calculation.md",
    "reference/structure-nodes-and-shapes.md",
    "reference/profile-types-sessions-and-vwap.md",
...
def test_frontmatter_has_name_and_description() -> None:
    ...
    These are the fields the skill loader uses to register and surface the skill;
    a missing key silently breaks discovery.
    """
...
def test_skill_name_is_volume_profile() -> None:
    """The skill's ``name`` must be exactly ``volume-profile``.
    ...
    assert name_match is not None and name_match.group(1) == "volume-profile"
...
def test_skill_md_relative_links_resolve() -> None:
    ...
    Catches typos and renames that would otherwise leave navigation broken.
    External (http) and anchor-only (#...) links are skipped. The cross-link to
    the sibling ``ict`` skill is therefore validated too.
    """
...
def test_reference_files_are_nonempty_and_cite_sources() -> None:
    ...
    Sourcing makes the Volume Profile claims auditable (researched, not invented);
    a near-empty file would signal a botched write.
    """
    for rel in REQUIRED_REFERENCES:
...
def test_key_conventions_documented() -> None:
    ...
    These specific items (the value-area algorithm, the NY-time/session rule, and
    the OHLCV approximation caveat) are the ones most likely to cause profile
    mismatches or wrong levels if dropped, so we assert they remain documented.
    """
    text = SKILL_MD.read_text(encoding="utf-8")
    assert "Value Area algorithm" in text, "value-area algorithm section missing"
    assert "America/New_York" in text, "session timezone convention missing"
    assert "footprint" in text, "OHLCV data-approximation caveat missing"
